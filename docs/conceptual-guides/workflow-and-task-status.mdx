---
slug: "/conceptual-guides/workflow-and-task-status"
description: "Learn the workflow and task statuses in Orkes Conductor and how they represent execution states such as RUNNING, COMPLETED, and FAILED."
---

# Workflow and Task Status

The workflow and task statuses are crucial to understanding the execution mechanism in Conductor. You can check a workflow execution’s current status and task status [from the Conductor UI](/content/developer-guides/debugging-workflows#using-conductor-ui) or [using API](/reference-docs/api/workflow/search-workflow-executions).
...
| FAILED | Terminal status where the workflow has encountered an error and failed. You can [retry the workflow execution from the failed task](/content/developer-guides/debugging-workflows#recovering-from-failure). |
...
| FAILED | The task failed to complete due to an error, and the workflow can be [retried from the failed task](/content/developer-guides/debugging-workflows#recovering-from-failure). |
