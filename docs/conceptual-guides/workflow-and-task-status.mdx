...
description: "Learn the workflow and task statuses in Orkes Conductor — RUNNING, COMPLETED, FAILED, and other execution states — and how they affect task lifecycle."
---
...
<figure>
  <p align="center">
    <img src="/content/img/task-states.png" alt="Task state transitions in Conductor." width="100%" height="auto" />
  </p>
  <figcaption align="center">A simplified view of the most common task state transitions.</figcaption>
</figure>
...
| ------ | ----------- | --------- | -------- |
| SCHEDULED | The task has been scheduled to be picked up by a worker. | Yes | No | 
| IN_PROGRESS | The task is being executed by a worker. | Yes | No | 
| SKIPPED | The task is skipped without executing, and the workflow continues to the subsequent tasks. <br/><br/> Occurs if the [Skip Task API](/reference-docs/api/workflow/skip-task-from-workflow) is used in a currently running workflow. | No | Yes | 
...
| FAILED | The task failed to complete due to an error, and the workflow can be [retried from the failed task](debugging-workflows#recovering-from-failures). | Yes | Yes | 
| FAILED_WITH_TERMINAL_ERROR | The task failed to complete due to an error and cannot be retried. | No | Yes | 
...
| COMPLETED | The task has been successfully completed without any errors. | Yes | Yes | 
