...
[Azure OpenAI has different models](https://learn.microsoft.com/en-us/azure/ai-services/openai/concepts/models), such as text-davinci-003, text-embedding-ada-002, gpt-35-turbo, and more, each designed for various use cases, such as text completion and embedding generation. Choose the model that best fits your use case.

When you deploy a model in the Azure portal, you are required to assign it a deployment name. This name is not the same as the model name (e.g., gpt-35-turbo). Instead, it is a unique identifier that your  integration uses to reference the model instance. 

:::info Prerequisites
- To use Azure OpenAI models with Orkes Conductor, you must first deploy the [desired models in your Azure portal](https://learn.microsoft.com/en-us/azure/ai-services/openai/how-to/create-resource?pivots=web-portal#deploy-a-model). Models are not available for use until they have been explicitly deployed in your Azure portal.
- Note the deployment name you assign, these will be referenced during integration. Once the deployment is complete, proceed with adding the model to Orkes Conductor.
:::
...
2. Select **+ New model**.
3. Enter the **Model name**, which is the deployment name used in the Azure portal. For example, if your deployment name is *mySummarizerModel*, enter *mySummarizerModel* here—not the base model name like gpt-35-turbo.
