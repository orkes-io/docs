...
5. Set access limits to the AI model to govern which applications or groups can use them.
...
Follow the Azure OpenAI documentation to [deploy the desired model in the Azure portal](https://learn.microsoft.com/en-us/azure/ai-services/openai/how-to/create-resource?pivots=web-portal#deploy-a-model). 

When deploying a model, you are required to assign a **deployment name**. This name serves as a unique identifier for the deployed model and is required when referencing the model in Orkes Conductor integration. It does not need to match the base model name (for example, gpt-35-turbo), though it can if desired.
