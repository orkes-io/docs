...
| Parameter | Description | Type | Required/Optional |
| --------- | ----------- | ---- | ----------------- |
| Text | The message text to send. The message is delivered to the channel configured in the webhook URL set up in [Step 1](/content/integrations/slack#get-the-webhook-url). | string | Required. | 
