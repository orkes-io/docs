---
slug: "/integrations/azure-storage-operations"
...
Returns a `containers` array with each container's `name` and `lastModified` timestamp, along with the total `count` of containers.
...
Returns the creation `status`, the `container` name, and the `url` of the newly created container.
...
Returns the deletion `status` and the `container` name of the deleted container.
...
Returns the `container` name, a `blobs` array with each blob's `name`, `size`, `lastModified` timestamp, `blobType`, and `contentType`, along with the total `count`, a `hasMore` flag indicating if more results exist, and a `continuationToken` for pagination.
...
Returns the upload `status`, `blob` name, `container` name, file `size`, `contentType`, `url` of the uploaded blob, any `metadata`, and the `sourceUrl` if uploaded from a URL.
...
Returns the upload `status`, `blob` name, `container` name, file `size`, `contentType`, `url` of the uploaded blob, any `metadata`, and the `sourceUrl` used to fetch the content.
...
Returns the `exists` flag, `blob` name, `container` name, `size`, `contentType`, `lastModified` timestamp, `eTag`, `blobType`, and `url` of the blob.
...
Returns the `blob` name, `container` name, `size`, and the `base64` encoded content of the downloaded blob.
...
Returns the deletion `status`, `blob` name, and `container` name of the deleted blob.
