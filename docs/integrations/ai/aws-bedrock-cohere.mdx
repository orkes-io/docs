...
To use system AI tasks in Orkes Conductor, you must integrate your Conductor cluster with the necessary AI/LLM providers. This guide explains how to integrate AWS Bedrock Cohere with Orkes Conductor. Here’s an overview:
...
To integrate AWS Bedrock Cohere with Orkes Conductor, retrieve the following credentials from your AWS account:
...
Once you’ve integrated AWS Bedrock Cohere, the next step is to configure specific models.

AWS Bedrock Cohere has different models, such as Command, Command Light, Command R, and more, each designed for various use cases. Choose the model that best fits your use case.
...
2. Select **+ New model**.
3. Enter the **Model name** and a **Description**. Get the complete list of [AWS Bedrock Cohere models](https://docs.aws.amazon.com/bedrock/latest/userguide/models-supported.html). 
