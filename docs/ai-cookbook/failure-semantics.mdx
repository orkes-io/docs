---
slug: "/ai-agents/failure-semantics"
...
**If you want to apply the new definition:** [Restart](/reference-docs/api/workflow/restart-workflow) only applies once an execution reaches a terminal state - it has no effect on a running workflow. Once an execution completes, fails, or is terminated, restart it with `useLatestDefinitions=true` to re-run it from the beginning using the updated definition, or [rerun it from a specific task](/reference-docs/api/workflow/rerun-workflow) with updated inputs.
...
- **[Production Agent Architecture](/ai-agents/production-agent-architecture)** - The canonical end-to-end agent pattern.
- **[Why Conductor for Agents](/why-conductor-for-ai-agents)** - What Conductor gives you out of the box for agentic workflows.
- **[Token Efficiency](/ai-agents/token-efficiency)** - How durable execution saves tokens across all these failure scenarios.
