---
slug: "/ai-agents/first-ai-agent"
description: "Build your first durable AI agent in 5 minutes with the Conductor Agent SDK on the free Developer Edition, with automatic crash recovery."
---

# Build Your First AI Agent

**Build a durable AI agent in 5 minutes.** Orkes Conductor includes native support for durable agents through the Conductor Agent SDK: your agent's reasoning loop, tool calls, and pauses are all persisted as a durable Conductor execution. If the process running your agent crashes, the server resumes it with no lost progress, no re-run LLM calls.
...
To view the agent execution in UI, open your Developer Edition cluster and go to [Agents > Executions](https://developer.orkescloud.com/agent-executions) to see the agent's execution. Open the latest run of the `weatherbot` execution. You can also view the underlying Conductor workflow that ran behind the hood.
...
- **[Durable Agents](/content/ai-agents/durable-agents)** — how Conductor's durable execution model applies to agent workflows generally, including patterns beyond the Conductor Agent SDK.
