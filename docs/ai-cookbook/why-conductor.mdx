---
slug: "/why-conductor-for-ai-agents"
...
Restart only applies once an execution reaches a terminal state — Conductor cannot swap the definition under a workflow that's still running. Once an execution completes, fails, or is terminated, you can [restart it with the latest definition](/reference-docs/api/workflow/restart-workflow) to re-run it from the beginning, or [rerun it from a specific task](/reference-docs/api/workflow/rerun-workflow) with updated inputs.
...
- **[Production Agent Architecture](/ai-agents/production-agent-architecture)** — The canonical end-to-end agent pattern, fully wired.
- **[Failure Semantics for AI Agents](/ai-agents/failure-semantics)** — The exact failure contract under every scenario.
- **[Build Your First AI Agent](/ai-agents/first-ai-agent)** — From zero to a running agent in 5 minutes.
