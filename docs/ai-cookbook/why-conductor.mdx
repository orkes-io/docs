...
This is where Conductor's JSON-native runtime is especially useful. An LLM generates a complete workflow definition as JSON. [Register it](/reference-docs/api/metadata/creating-workflow-definition), then start it by name:
...
```json
{
  ...
  "inputParameters": {
    "startWorkflow": {
      "name": "${planner_llm.output.result.workflowName}",
...
The LLM's output is a Conductor workflow definition — register it, then start it by name. No code generation. No compilation. No custom deployment pipeline to build and maintain. The generated workflow runs with the same durable execution guarantees as any hand-written workflow — persistence, retries, observability, replay.
...
| **Agent that builds its own plan** | `LLM_CHAT_COMPLETE` + `START_WORKFLOW` for a registered generated workflow |
| **Deterministic post-processing** | `INLINE` (JavaScript) or `JSON_JQ_TRANSFORM` |


## Next steps

- **[Production Agent Architecture](/ai-orchestration/production-agent-architecture)** — The canonical end-to-end agent pattern, fully wired.
- **[Failure Semantics for AI Agents](/ai-orchestration/failure-semantics)** — The exact failure contract under every scenario.
- **[Build Your First AI Agent](/ai-orchestration/first-ai-agent)** — From zero to a running agent in 5 minutes.
