---
slug: "/ai-agents/token-efficiency"
...
When a workflow fails (e.g., a tool call returns an error after the LLM planned successfully), you can [retry the failed workflow](/reference-docs/api/workflow/retry-failed-workflow). Conductor reuses the outputs of all previously completed tasks.
...
When you fix a bug in a task definition and [rerun from that task](/reference-docs/api/workflow/rerun-workflow), all tasks before it keep their persisted outputs. Upstream LLM calls are not re-executed.
...
This is the same persistence model that applies to every task in Conductor - the [durable execution semantics](/quickstarts/durable-execution) guarantee that completed work is never lost.
...
- **[Durable Execution Semantics](/quickstarts/durable-execution)** - The full persistence and recovery model.
- **[LLM Orchestration](/ai-orchestration/llm-orchestration)** - 12 native LLM providers, vector databases, and RAG pipelines.
