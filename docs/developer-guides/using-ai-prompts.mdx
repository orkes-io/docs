...
description: "Learn how to create and manage prompt templates with variables and reuse them in AI tasks across workflows."
---
...
| Model           | Select the AI models to use with this prompt.<br/><br/><TableNote title="Note">Only models added here can generate responses based on this prompt in a workflow.</TableNote> |
...
| Stop words           | Enter one or more stop sequences. The model stops generating text when it produces any configured sequence. For example, `END` stops generation when the model produces that sequence. |
