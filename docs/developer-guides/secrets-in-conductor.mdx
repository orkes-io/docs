...
Updating secrets can be challenging when tokens expire and require periodic updates. Use the [Update Secret](https://orkes.io/content/reference-docs/system-tasks/update-secret) task in Conductor to replace such secrets within a workflow.
...
When this workflow runs, the expression `${workflow.secrets.sampletask-api-token}` will be dynamically replaced with the secret value, provided the user running the workflow has [permission](/content/category/access-control-and-security) to access it.
