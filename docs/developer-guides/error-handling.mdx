...
:::note
To deal with workflow failures post-execution, refer to [Debugging Workflow Executions](/developer-guides/debugging-workflows).
:::
...
This durability guarantee also means worker code must be idempotent, since a retried worker should be safe to run again with the same task input.
...
- Use [Metrics and Observability](/developer-guides/metrics-and-observability) to monitor retries, timeouts, queue depth, and failure rates.
- Use [Debugging Workflow Executions](/developer-guides/debugging-workflows) to inspect the failed task, resolved inputs, and recovery options.
