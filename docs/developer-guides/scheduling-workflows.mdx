...
import TableNote from '@site/src/components/TableNode';
...
| startworkflowRequest. **idempotencyKey** | A unique, user-generated key to prevent duplicate workflow executions. Idempotency data is retained for the life of the workflow execution. <br/><br/><TableNote>The `idempotencyKey` is optional. If not provided, the workflow will be executed without checking for duplicate executions.</TableNote>  | Optional. | 
