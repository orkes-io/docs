...
When building workflows, you can use the built-in system tasks and operators provided by Conductor and additionally [write your own custom worker tasks](using-workers).
...
Although each task type has its unique configuration, all tasks share several parameters in common. Refer to the [Task Reference](/category/reference-docs) to learn more about the task configuration for each task type.
...
Data can be passed from one task to another, by using dynamic references in a task’s `inputParameters`. These dynamic references are formatted as JSONPath expressions. Refer to [Wiring Parameters](passing-inputs-to-task-in-conductor) to learn more.
