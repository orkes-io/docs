...
There is also an option to restart execution using the latest definitions rather than those at the initial execution time. In this case, the V1 execution will restart using the definition at T2 instead. 

Learn more about [workflow restarts and retries](https://orkes.io/content/developer-guides/debugging-workflows#recovering-from-failures).
