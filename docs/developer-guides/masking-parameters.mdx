...
Sensitive data can be masked using either _\_secrets_ or _\_masked_ objects in workflow definitions. Any values stored in objects with keys _\_secrets_ or _\_masked_ will be replaced with **\*\*\*** in the workflow executions. Users with execute permission for the task can read the inputs and outputs.
...
```json
{
  "_secrets": { "parameter": "${previousTaskRef.output.someOutputParameter}" }
}
```
...
This ensures that any input parameter to be hidden must be nested within the _\_secrets_ object, ensuring that it is masked adequately during the workflow execution without exposing it.
...
Consider a workflow with a task having an input masked using _\_secrets_:
...
Consider a workflow with a task having an input masked using _\_masked_:
