...
- Values masked with `_masked` are retained during archiving. Restarting the workflow preserves the original data.
- Values masked with `_secrets` are permanently replaced with `***` during archiving. Restarting the workflow may cause failures if tasks depend on the masked data.
- Values masked with `maskedFields` are permanently replaced with `***` during archiving. Restarting the workflow may cause failures if tasks depend on the masked data.
