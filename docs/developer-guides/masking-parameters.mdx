...
Fields that reference workflow secrets using the `${workflow.secrets.<secretName>}` syntax are masked during execution. When these expressions are resolved, their values are replaced with *** anywhere they appear, including workflow inputs, task inputs, task outputs, and the execution JSON.
  ...
  "tasks": [
    {
      ...
      "type": "HTTP",
      "inputParameters": {
        "uri": "https://payments.example.com/api/charge",
        "method": "POST",
        "headers": {
...
The Authorization header value is masked because it resolves from `${workflow.secrets.payment_api_token}`. Any occurrence of this secret value elsewhere is also masked automatically.
