...
4. Turn on the **Cache output** toggle and enter the **TTL (in seconds)** and **Cache key**. Refer to [Configuration](#configuration) for more information on the parameters.
...
| -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------ |
| **TTL (in seconds)** | The time to live in seconds, which is the duration for the output to be cached.                                                                                                                                                                                        | Required.          |
| **Cache key**        | The cache key is a unique identifier for the cached output and must be constructed exclusively from the task’s input parameters. <br/><br/> It can be a string concatenation that contains the task’s input keys, such as `${uri}-${method}` or `re_${uri}_${method}`. | Required.          |
