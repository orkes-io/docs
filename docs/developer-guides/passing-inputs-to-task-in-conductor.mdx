...
description: "Learn how to wire parameters in Orkes Conductor using dynamic expressions to pass data between workflows, tasks, variables, secrets, and sub-workflows, plus task input templates, masking, and schema validation."
---
...
:::tip 5-minute path
Start with `${workflow.input.<field>}` for request data, `${task_ref.output.<field>}` for prior task output, `${workflow.variables.<field>}` for shared workflow state, and `${workflow.secrets.<name>}` for credentials. Use [task input templates](#task-input-templates) for shared defaults, [masking](#masking-sensitive-mapped-values) for sensitive values, and [schema validation](#schema-validation) for production contracts.
:::
...
## Beyond parameter references

The dynamic references above cover most day-to-day wiring. Conductor also provides three related mechanisms for managing task inputs at scale: reusable **task input templates**, **masking** for sensitive mapped values, and **schema validation** for enforcing input/output contracts.
...
For credentials stored centrally, referencing `${workflow.secrets.<secretName>}` (see [Referencing secrets](#examples) above) already masks the value as `***` automatically. The mechanisms below cover other cases: runtime-sensitive values passed between tasks, and workflow-level fields that need masking by name.
