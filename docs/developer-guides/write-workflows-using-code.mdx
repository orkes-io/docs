...
:::tip 5-minute path
Define the workflow in your SDK, register it with Conductor, start an execution, then implement any custom task as a [worker](/developer-guides/using-workers). Workflow code describes the graph; worker code performs the business logic.
:::
