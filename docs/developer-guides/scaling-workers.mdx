...
Conductor server publishes following metrics per task queue:
(Examples uses [PromQL](https://prometheus.io/docs/prometheus/latest/querying/basics/))

:::tip
Each of the below metric contains `taskType` as a tag that can be used to monitor the metric for a specific task.
:::
...
:::tip
These are the three critical metrics that helps you tune your worker configuration for the optimal throughput of the system.
:::
