...
Running a workflow means creating a workflow execution from a saved workflow definition. The key decision is how it gets triggered: synchronously (the caller waits for a result), asynchronously (fire and forget), on a schedule, or in response to an external event.
