...
You'll need a version of Conductor.  The images will be based on the [ Developer Playground](https://developer.orkescloud.com), but a [local Conductor instance](http://localhost:3000/content/get-orkes-conductor) works as well.
...
The workflow is pretty basic - with a name (if using the Developer Playground, your workflow will need a unique name). And description.  There are 2 ```outputParameters``` defined - that we'll discuss further in the codelab.
