...
If you want to run the workflow quickly, you can try to run them in Orkes Developer Playground. Since you haven’t set up the worker here, our Orkes workers will complete the task. Let’s see how to run this in the Developer Playground. 

| [Run Video Recipe in Developer Playground](https://developer.orkescloud.com/runWorkflow) |
|------------------------------------------------------------------------------| 
...
Replace the above values with your credentials. If you are testing this in the Developer Playground, **conductor.server.url** field remains the same. If you are using your own Conductor server, replace the Developer Playground URL with your server URL.
...
This configuration ensures the connection between the worker and the Developer Playground/Conductor.
