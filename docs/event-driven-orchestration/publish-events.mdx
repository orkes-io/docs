...
description: "Publish messages from workflows to external event systems such as Kafka, Confluent Kafka, Amazon MSK, AMQP/RabbitMQ, NATS, AWS SQS, Azure Service Bus, GCP Pub/Sub, and IBM MQ, plus internal Conductor event sinks."
---
...
The Event task publishes a message to a configured sink — an internal Conductor event queue, or an external system such as Kafka, SQS, or another broker — as a step in the workflow, without waiting for a response. Use it when a downstream system or workflow needs to be notified as part of durable execution, and see the [Event Task reference](/reference-docs/system-tasks/event) for the full input/output schema, sink configuration, and examples.
