...
Orkes Conductor provides role-based access control (RBAC) for individual users and applications that interact with Conductor APIs. RBAC ensures fine-grained access to the following metadata resources:
...
To get started with RBAC, learn about users, groups, applications, tags, roles, and permissions.
...
Tags make it easier to share permissions across many resources or users. When you grant tag permissions to a group or application, it provides access to all resources that contain that tag.
