...
There is no concept of defining a tag. Any key:value pair may be used as a tag.

## Adding a Tag to a Workflow

There are several ways to add a tag to a workflow. In this section, choose between using the API or the Dashboard for detailed instructions on both options.
...
```bash
curl -X PUT "https://play.orkes.io/api/metadata/workflow/neptune_status/tags" \
-H  "accept: */*" \
-H  "X-Authorization: <access_token>" \
-H  "Content-Type: application/json" \
-d "[{\"type\":\"METADATA\",\"key\":\"rings\",\"value\":\"true\"}]"
```
...
```bash
curl -X GET "https://play.orkes.io/api/metadata/workflow/neptune_status/tags" \
-H  "accept: */*" \
-H  "X-Authorization: <access_token>" \
-H  "Content-Type: application/json" \
```
...
```bash
curl -X DELETE "https://play.orkes.io/api/metadata/workflow/neptune_status/tags" \
-H  "accept: */*" \
-H  "X-Authorization: <access_token>" \
-H  "Content-Type: application/json" \
-d "[{\"type\":\"METADATA\",\"key\":\"rings\",\"value\":\"true\"}]"
