...
5. Set the `base_url`, Key ID, and Key Secret in `main.py`, located at `conductor-apps/python/developer-guides/using-workers/main.py`. Note that `base_url` should not include `/api`.
...
Using the command line, navigate to the downloaded project directory and launch the worker. You can follow the README in the sample project for language-specific instructions.
