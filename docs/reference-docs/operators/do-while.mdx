...
| inputParameters. **keepLastN** | The number of required iterations. On enabling this option, this value is set to 2 by default. You can also choose not to include this parameter if there are no limits on the number of iterations.                | Optional. Do not use if `inputParamters.items` is set.         |
| inputParameters. **items** | An array of items, which will be iterated over in the loop. The number of iterations is equal to the list size. You can use this to iterate without a condition. Refer to [Examples](#examples) for more information on how to use the `items` parameter.            | Optional.          |
