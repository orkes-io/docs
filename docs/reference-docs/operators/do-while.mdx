...
In addition, an object will be created for each iteration, keyed by its iteration number (e.g., 1, 2, 3), and will contain the task outputs for all of the `loopOver` tasks. If `keepLastN` is specified, only outputs for the last _n_ iterations will be stored.
...
Meanwhile, the output for the Do While task itself will contain the outputs for every iteration:
