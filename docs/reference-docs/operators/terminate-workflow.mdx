...
description: "Learn how the Terminate Workflow task terminates another workflow execution from the current workflow."
---
...
The user or application that starts the workflow containing the **_Terminate Workflow_** task must have **_Execute_** permission on each workflow to be terminated. Otherwise, the task fails.
...
| -------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------ |
| inputParameters. **workflowId**        | The workflow (execution) IDs of the workflow executions to be terminated. Can be a single ID or an array of one or more IDs. It can be [passed as a dynamic variable](/developer-guides/passing-inputs-to-task-in-conductor).                                           | Required.          |
| inputParameters. **terminationReason** | The reason for terminating the workflow(s), which will provide the context of the termination. It can be [passed as a dynamic variable](/developer-guides/passing-inputs-to-task-in-conductor). If not set, it defaults to `Terminated by task <taskId>`, where `<taskId>` is the task (execution) ID of the Terminate Workflow task.                              | Optional.          |
| inputParameters. **triggerFailureWorkflow** | Whether the failure workflow for the terminated workflow will be triggered. Accepted values:<ul><li>`true`: The failure workflow will be triggered.</li><li>`false`: The default option. The failure workflow will not be triggered.</li></ul> | Optional.          |

The task terminates the workflows in the order they are listed and stops at the first workflow it cannot terminate, such as a workflow that has already completed. When this happens, the task fails and returns no output. Workflows earlier in the list remain terminated, and the rest of the list is not processed. Workflows that are already terminated do not cause the task to fail.
...
```json
{
     ...
     "inputParameters": {
       "workflowId": "${workflow.input.workflowIds}",
...
| ------------------- | ----------------------------------------------------------------------- |
| terminatedWorkflows | A JSON-encoded string containing an array of the workflow (execution) IDs that were terminated, for example, `"[\"id-1\",\"id-2\"]"`. Duplicate IDs are removed, and the order may differ from the input. |
...
<ol start="4">
<li>Save the workflow and select <strong>Execute</strong> to run it.</li>
</ol>
...
To verify this, go to **Executions** > **Workflow** and search for the terminated workflow ID. Select the workflow ID to view the execution. At the top of the execution details, you can view the termination reason that was provided in the Terminate Workflow task.
