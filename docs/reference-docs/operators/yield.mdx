...
The Yield task temporarily yields control over the workflow by pausing the execution until it is explicitly resumed using the [sync](/content/reference-docs/api/task/signal-running-task-synchronously) or [async](/content/reference-docs/api/task/signal-running-task-asynchronously) signal endpoints, typically in response to external conditions or events.

It is similar to the [Wait task](https://orkes.io/content/reference-docs/operators/wait), which also introduces a pause point in a workflow. However, unlike the Wait task, the Yield task must be explicitly completed using the [sync](/content/reference-docs/api/task/signal-running-task-synchronously) or [async](/content/reference-docs/api/task/signal-running-task-asynchronously) signal endpoints and supports passing data back into the workflow when resumed. This enables workflows to continue in flight with updated inputs.
...
| --------- | ----------- | ----------------- | 
| inputParameters | The input parameters for the Yield task, which can be fixed or [passed as a variable](https://orkes.io/content/developer-guides/passing-inputs-to-task-in-conductor). | Required. | 
...
| Parameter | Description | Required/Optional | 
| --------- | ----------- | ----------------- | 
| optional | Whether the task is optional. <br/> If set to true, the workflow continues to the next task even if this task fails or remains incomplete. | Optional. | 
