...
description: "Learn how the Yield task temporarily pauses workflow execution and returns control to the caller."
---
...
The signal endpoints update the first blocking Wait or Yield task in the workflow, including tasks in nested sub-workflows. By default, the Yield task has no timeout and remains in progress until it is signaled.
...
| --------- | ----------- | ----------------- | 
| inputParameters | The input parameters for the Yield task, which can be fixed or [passed as a dynamic variable](/developer-guides/passing-inputs-to-task-in-conductor). | Optional. | 
...
You can cache task outputs using the following parameters. Only the outputs of completed tasks are cached. If a cached output exists for the computed cache key, the Yield task is completed immediately with the cached output, without waiting to be signaled, and the output includes `"_cachedResponse": true`. Refer to [Caching Task Outputs](/faqs/task-cache-output) for a full guide.
...
```shell
curl -X 'POST' \
  'https://<YOUR-CLUSTER-NAME>/api/tasks/<WORKFLOW-ID>/COMPLETED/signal' \
  -H 'accept: */*' \
