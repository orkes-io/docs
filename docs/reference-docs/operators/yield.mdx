...
When used in workflows that run in synchronous consistency mode, and when the execution reaches the Yield task, Conductor returns a response immediately as defined by the ‘returnStrategy’ in Execute Workflow Synchronously endpoint.
