...
| defaultCase   | The default branch. Contains the list of tasks to be executed when no matching value is found in the decision cases. Use `[]` if no fallback is needed.<br/><br/>If `defaultCase` is `[]` and no case matches, the Switch task completes successfully and the workflow continues to the next task without executing any branch tasks. | Required.          |
