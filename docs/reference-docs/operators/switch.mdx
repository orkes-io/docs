...
<details>
<summary>Nested switch case</summary>
<p>
...
<details>
<summary>JavaScript expressions​</summary>
<p>
When using `javascript` or `graaljs` as the evaluator type, the `expression` can be a JavaScript expression that returns a string. Within the `expression`, the Switch task input parameter is available as a dynamic input inside the $ scope.
...
<details>
<summary>JavaScript expression that evaluates a datetime string</summary>
<p>
