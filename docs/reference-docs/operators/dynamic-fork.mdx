...
A Join task must follow a Dynamic Fork task to complete the fork-join operation.

- The Join task implicitly waits for all dynamically created forks.
- You can set `joinOn` to an empty array.
- The Join task must still include the required schema fields.
