...
The [Join](./join) task will run after the forked tasks and implicitly waits for all forked tasks to complete. For a Dynamic Fork, no specific configuration is required, but the Join task must still be included in the workflow to complete the fork-join operation.
...
3. Select the Join task and configure its settings to complete the fork/join operation.
