...
- A [dynamic input](/content/developer-guides/passing-inputs-to-task-in-conductor) that is defined in real-time during workflow execution.
...
In the example above, `set_name` is a Set Variable task that takes a dynamic input for `name`. In subsequent tasks, the `name` is later referenced using `${workflow.variables.name}`.
