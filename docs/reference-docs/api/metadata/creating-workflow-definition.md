...
| workflowStatusListenerEnabled | Setting this value to **true**, will enable workflow status change events to sink if configured. For more information  [workflow status change events]([https://orkes.io/content/templates/document-classifier](https://orkes.io/content/developer-guides/task-and-workflow-event-streaming-with-orkes-conductor-and-azure-event-hub#publishing-task--workflow-status-change-stream-events))                                     |
