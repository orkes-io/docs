...
| sort | The field used to sort the results. Supported values:<ul><li>`name`</li><li>`workflowname`</li><li>`createtime`</li><li>`updatedtime`</li><li>`startworkflowrequest` (sorts by workflow name)</li></ul>If no value is specified or if the value doesn’t match one of these fields, results are sorted by `name` (schedule name) by default. | string | Optional. | 
...
| freeText | The text to search for. Default is `*`. | string | Optional. | 
...
<summary>Sort the schedule definitions by pagination</summary>
