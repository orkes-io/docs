...
Starts a workflow execution synchronously, and returns a response after a specified *waitForSeconds* has passed or when a specified *waitUntilTaskRef* task completes, whichever completes first. Set **consistency** to **SYNCHRONOUS** to execute the workflow directly from memory without persisting the request.
