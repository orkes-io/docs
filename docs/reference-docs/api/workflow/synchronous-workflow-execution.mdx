...
description: "Learn how to start a workflow execution and wait for the workflow state or output in the response."
---
...
For workflows containing a Wait task with a `duration` set, the response is held until `waitForSeconds` expires. Set `yield: true` in the Wait task's `inputParameters` to return an immediate response instead. [Learn more](/content/reference-docs/operators/wait).
