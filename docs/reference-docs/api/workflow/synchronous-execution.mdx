...
# Execute Workflow Synchronously (Deprecated)
...
Starts a workflow execution synchronously. This method returns a response after a specified _waitForSeconds_ has passed or when a specified _waitUntilTaskRef_ task completes, whichever completes first.
