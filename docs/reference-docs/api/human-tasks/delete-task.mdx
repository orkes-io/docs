...
| --------- | ----------------------------------------------------------------- | ------ | ------------------ |
| taskId    | The task (execution) ID for the Human task execution to be deleted. | string | Required.          |
...
```shell
curl -X 'DELETE' \
  'https://<YOUR-CLUSTER>/api/human/tasks/delete/7f49e3c7-ce62-11ef-a89d-86a819bd92bf' \
  -H 'accept: */*' \
