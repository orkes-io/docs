...
| --------- | ------------------------------------------------------------------ | ------ | ------------------ |
| taskId    | The task (execution) ID for the Human task execution to be released. | string | Required.          |
...
```shell
curl -X 'POST' \
  'https://<YOUR-CLUSTER>/api/human/tasks/869ed0ee-cf07-11ef-a89d-86a819bd92bf/release' \
  -H 'accept: */*' \
