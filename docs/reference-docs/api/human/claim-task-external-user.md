...
Used to claim a human task by any users. Use this API to claim a task on behalf of a user which can be external or
internal.

This is the API for applications which are built on top of Conductor to retrieve and act on tasks for users. For example
if user "A" is logged on to the system in application "APP", then "APP" can retrieve the tasks eligible for user "A" to
act and can claim a task if the user "A" is ready to act on it. "APP" needs to use a credential that is an owner of the
...
will assume the external system has validated the claim criteria. If the task is assigned using __Free for all__
policy, any user will be able to retrieve the task. When using application credentials, the application should be an
