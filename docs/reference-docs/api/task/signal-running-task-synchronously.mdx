...
For workflows containing a Wait task with a `duration` set after receiving a signal, the server holds the connection open until `timeoutMillis` expires. Set `yield: true` in the WAIT task's `inputParameters` to return an immediate response instead. [Learn more](/content/reference-docs/operators/wait).
