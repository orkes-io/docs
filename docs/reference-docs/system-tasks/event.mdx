...
| asyncComplete   | When enabled, the task remains in an IN_PROGRESS state while it awaits completion from external intervention, allowing asynchronous task completion. <TableNote title="Available since">v4.1.40 and later</TableNote> | Optional. | 
