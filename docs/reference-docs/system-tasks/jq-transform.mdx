...
The HTTP task returns a JSON response that includes an array. Each entry includes fields such as `created_at`, `author`, and `title`, along with many other fields.
