...
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------ |
| inputParameters.**matches** | The conditions that incoming event payloads must meet to trigger the webhook. Can be string, number, boolean, null, or object/array.<br/><TableNote title="Note">When you update the matches, there will be a default caching period of 60 seconds, causing the updates to reflect with a delay of up to 60 seconds.</TableNote> | Required.          |
...
See an example of [using SendGrid Event Webhooks in Orkes Conductor](https://orkes.io/content/tutorial/using-sendgrid-webhooks).
