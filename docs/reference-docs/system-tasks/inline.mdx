...
:::info Note
If your Inline expression references a workflow secret (such as `${workflow.secrets.<secret-name>}`), the resolved value is masked in the task output. For details on masking behavior, see [Masking Parameters](https://orkes.io/content/developer-guides/masking-parameters#masking-sensitive-data).
:::
