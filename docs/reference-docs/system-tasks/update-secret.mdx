...
| inputparameters.**\_secrets.secretKey**   | The name of the secret key to be updated. It can be [passed as a dynamic input](/developer-guides/passing-inputs-to-task-in-conductor). | Required.          |
| inputparameters.**\_secrets.secretValue** | The new value for the secret key. It can be [passed as a dynamic input](/developer-guides/passing-inputs-to-task-in-conductor).         | Required.          |
