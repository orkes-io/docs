...
These workers communicate with the Conductor server via REST/gRPC, allowing them to poll for tasks and update the task status. Learn more in [Architecture](/content/conductor-architecture).
...
See [Task Definitions](/content/reference-docs/task-definition) for the full reference.
...
- **Domain isolation** — Use [task domains](/content/reference-docs/api/task-domains) to route tasks to specific worker groups.
