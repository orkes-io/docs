...
=== "Input Parameters"
    ...
    | Values | A 2D array of rows and values to append to the sheet. Each row is a numbered block, and each value within a row corresponds to a cell in that row.<br/>**To configure in the UI:**<ol><li>Select **+** at the top of the **Values** field to add a row.</li><li>Within the row, select **+** to add a cell value. Repeat to add more values to the same row.</li><li>Select **+** at the top again to add another row. Repeat as needed.</li></ol>**Example**: To append two rows where each row has a Name and Age, configure row 1 with **Name** and **Age**, and row 2 with **Alice** and **30**. | array of arrays | Required. | 
