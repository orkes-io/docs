...
When a workflow fails (e.g., a tool call returns an error after the LLM planned successfully), you can [retry from the failed task](/content/quickstarts/durable-execution#replay-and-recovery). Conductor reuses the outputs of all previously completed tasks.
...
When you fix a bug in a task definition and [rerun from that task](/content/quickstarts/durable-execution#replay-and-recovery), all tasks before it keep their persisted outputs. Upstream LLM calls are not re-executed.
...
- **[Durable Agents](/content/ai-orchestration/durable-agents)** — What persists, what gets retried, and why JSON is AI-native.
- **[Durable Execution Semantics](/content/quickstarts/durable-execution)** — The full persistence and recovery model.
- **[Build Your First AI Agent](/content/ai-orchestration/first-ai-agent)** — Step-by-step tutorial with durable execution built in.
- **[LLM Orchestration](/content/ai-orchestration/llm-orchestration)** — 14+ native LLM providers, vector databases, content generation.
