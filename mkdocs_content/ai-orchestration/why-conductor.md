...
Running executions continue with their original definition. New executions pick up the updated definition. No version guards. No branching. No archaeology. Update the definition, register it, and move on. If you need to apply the new definition to a running execution, [restart it](/content/quickstarts/durable-execution#replay-and-recovery) — Conductor re-executes the workflow with the latest definition from the beginning.
...
- **[Production Agent Architecture](/content/ai-orchestration/production-agent-architecture)** — The canonical end-to-end agent pattern, fully wired.
- **[Failure Semantics for AI Agents](/content/ai-orchestration/failure-semantics)** — The exact failure contract under every scenario.
- **[Build Your First AI Agent](/content/ai-orchestration/first-ai-agent)** — From zero to a running agent in 5 minutes.
