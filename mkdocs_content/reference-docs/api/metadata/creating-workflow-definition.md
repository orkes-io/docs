...
description: "Use the Orkes Conductor metadata API to create Workflow Definition. Includes endpoint details, authentication, parameters, request bodies, response behavior."
canonical_route: "reference-docs/api/metadata/creating-workflow-definition"
updated: "2026-05-14"
keywords: "Orkes Conductor, workflow orchestration, Create Workflow Definition, Create Workflow Definition API, API orchestration, API gateway"
---
...
| [timeoutSeconds](https://orkes.io/content/error-handling#timeout-configuration_1)            | The timeout, in seconds, after which the workflow will be set as TIMED_OUT if it hasn't reached a terminal state. No timeout occurs if the value is set to 0.                                                                                                                                                                                                                                                                                                                                                                                  | integer          | Required.                                              |
| [timeoutPolicy](https://orkes.io/content/error-handling#timeout-configuration_1)             | The policy for handling workflow timeout. Supported values:<ul><li>**TIME_OUT_WF**: The workflow is set to TIMED_OUT and is terminated.</li><li>**ALERT_ONLY**: Increments the counter to check the workflow status when it times out and logs relevant messages.</li></ul>                                                                                                                                                                                                                                                                | string           | Optional.                                              |
...
| [rateLimitConfig](https://orkes.io/content/rate-limits#rate-limit-configuration_1)        | A map of the workflow rate limit configuration.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | object           | Optional.                                              |
| rateLimitConfig. [**rateLimitKey**](https://orkes.io/content/rate-limits#rate-limit-configuration_1)                                                      | A unique identifier to group workflow executions for rate limits.<br/>Can be a fixed value (for example, "max") or a dynamic variable from the workflow input (for example, `${workflow.input.correlationId}`).                                                                                                                                                                                                                                                                                                                        | string           | Optional.                                              |
| rateLimitConfig. [**concurrentExecLimit**](https://orkes.io/content/rate-limits#rate-limit-configuration_1)                                               | The number of workflow executions that can run concurrently for each rate limit key. Cannot be passed as a dynamic variable.                                                                                                                                                                                                                                                                                                                                                                                                                                                           | integer          | Optional.                                              |
| [maskedFields](https://orkes.io/content/developer-guides/masking-parameters)                                                                           | The fields to mask in the workflow execution. | array of strings | Optional. | 
...
```bash
curl -X 'POST' \
  'https://<YOUR-SERVER-URL>/api/metadata/workflow?overwrite=false&newVersion=false' \
  -H 'accept: */*' \
...
<details>
<summary>Create a new workflow definition with `newVersion` set to true</summary>

**Request**

```json
curl -X 'POST' \
  'https://<YOUR-SERVER-URL>/api/metadata/workflow?overwrite=false&newVersion=true' \
  -H 'accept: */*' \
  -H 'X-Authorization: <TOKEN>' \
  -H 'Content-Type: application/json' \
  -d '{
