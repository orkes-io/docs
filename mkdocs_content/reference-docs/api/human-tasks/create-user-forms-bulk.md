...
updated: "2026-05-14"
keywords: "Orkes Conductor, workflow orchestration, Create User Forms in Bulk, Create User Forms in Bulk API, API orchestration, API gateway"
---
...
Use this human tasks endpoint to create User Forms in Bulk. It is intended for automation scripts, CI/CD jobs, backend services, and internal tools that need to manage Orkes Conductor programmatically.

- **Method and path**: `POST` `/api/human/template/bulk`
...
```shell
curl -X 'POST' \
  'https://<YOUR-SERVER-URL>/api/human/template/bulk?newVersion=false' \
...
```shell
curl -X 'POST' \
  'https://<YOUR-SERVER-URL>/api/human/template/bulk?newVersion=true' \
  -H 'accept: application/json' \
  -H 'X-Authorization: <TOKEN>' \
  -H 'Content-Type: application/json' \
  -d '[
