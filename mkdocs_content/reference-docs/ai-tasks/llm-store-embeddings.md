---
title: "LLM Store Embeddings"
description: "Learn how the LLM Store Embeddings task stores generated embeddings in a vector database for later retrieval in Orkes Conductor."
canonical_route: "reference-docs/ai-tasks/llm-store-embeddings"
updated: "2026-05-14"
keywords: "Orkes Conductor, workflow orchestration, LLM Store Embeddings, LLM Store Embeddings AI task, AI orchestration, LLM orchestration, agent workflows"
---

# LLM Store Embeddings

The LLM Store Embeddings task is used to store the generated embeddings produced by the [LLM Generate Embeddings](/content/reference-docs/ai-tasks/llm-generate-embeddings) task in a vector database. The stored embeddings serve as a repository of information that can be later accessed by the [LLM Get Embeddings](/content/reference-docs/ai-tasks/llm-get-embeddings) task for efficient and quick retrieval of related data.
...
| inputParameters.**embeddingModelProvider** | The LLM provider for generating the embeddings. <br/><br/>**Note**: If you haven’t configured your AI/LLM provider on your Orkes console, navigate to the **Integrations** tab and [configure your required provider](/content/category/integrations/ai-llm). | Required. | 
...
```json
{
     ...
     "inputParameters": {
       ...
       "embeddings": "${llm_generate_embeddings_ref.output.result}",
       "metadata": {
         "SomeKey": "Some-value"
       }
     },
